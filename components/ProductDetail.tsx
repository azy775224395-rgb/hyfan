...
const ProductDetail: React.FC<ProductDetailProps> = ({ product, onClose, onAddToCart }) => {
  ...
  return (
    <div className="bg-white rounded-[3rem] overflow-hidden shadow-sm border border-emerald-50 flex flex-col md:flex-row min-h-[70vh] animate-fade-in relative">
      {/* Back Button */}
      <button 
        onClick={onClose} 
        className="absolute top-6 left-6 z-10 bg-white border border-gray-100 p-3 rounded-2xl hover:bg-emerald-50 hover:text-emerald-600 hover:border-emerald-100 shadow-sm transition-all flex items-center gap-2 font-bold text-sm"
      >
      ...
      {/* Image Section */}
      <div className="w-full md:w-1/2 bg-gray-50 flex items-center justify-center min-h-[400px]">
        <img 
          src={product.image} 
          alt={product.name} 
        ...
        <div className="space-y-10 flex-grow">
          ...
          {product.specs && (
            <div>
              ...
              {/* New More Info Button after Specs */}
              <button 
                onClick={handleMoreInfoWhatsApp}
                className="w-full bg-white border-2 border-emerald-600 text-emerald-700 py-4 px-6 rounded-2xl font-black flex items-center justify-center gap-3 hover:bg-emerald-50 transition-all active:scale-95 group"
              >
                <svg xmlns="http://www.w3.org/2000/svg" width="22" height="22" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2.5" strokeLinecap="round" strokeLinejoin="round" className="group-hover:rotate-12 transition-transform"><circle cx="12" cy="12" r="10"/><path d="M12 16v-4"/><path d="M12 8h.01"/></svg>
                طلب المزيد من المعلومات حول هذا المنتج
              </button>
            </div>
          )}
        </div>
