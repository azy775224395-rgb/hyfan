...
const BestSellersBar: React.FC<BestSellersBarProps> = ({ products, onViewDetails, formatPrice, onAddToCart }) => {
  ...
  useEffect(() => {
    if (isHovered || bestProducts.length === 0) return;
  ...
  }, [isHovered, bestProducts]);

  return (
    <div className="w-full bg-white border-b border-emerald-100/50 overflow-hidden py-1.5 shadow-[0_4px_20px_-10px_rgba(0,0,0,0.05)]">
       <div className="flex items-center justify-between px-2 md:px-4 mb-1.5">
         <h2 className="text-sm md:text-base font-black flex items-center gap-1.5 text-gray-900 border-r-2 border-red-500 pr-2">
         ...
         {bestProducts.map((product) => (
           ...
           >
             <div className="w-full h-24 md:h-28 relative bg-gray-50/50 overflow-hidden p-2">
               ...
               />
               <span className="absolute top-1.5 right-1.5 bg-red-600 text-white text-[8px] font-black px-1.5 py-0.5 rounded-full shadow-sm z-10">
                 الاكثر مبيعاً
               </span>
             </div>
             <div className="p-1.5 md:p-2 space-y-1 bg-white">
               ...
               <div className="pt-1 border-t border-gray-50 flex items-center justify-between">
                 <span className="text-red-600 font-black text-xs md:text-sm">
